        ...
        for juno_rgb in range(0, 1, 1):
            ...
            for roi_size in range(roi_size_beg, roi_size_max, roi_size_incr):
                ...
                if (write_ct == write_freq):
                    ...
                    fig = px.imshow(oc_img_acc_mad_sc_ctrpt, color_continuous_scale='gray')
                    fig.update_layout(
                        title="Center Points After Rescaling"
                    )
                    fig.show()
                    fig = px.imshow(oc_img_acc_mad_sc_ctrpt_sum_over_sizes, color_continuous_scale='gray')
                    fig.update_layout(
                        title="Center Points After Rescaling: Sum Over Sizes"
                    )
                    fig.show()
                    ...
                    # fig.show()
                    # fig.update_layout(title="Histogram: Center Points After Rescaling")

                    # Center point mad this roi:

                    fig = px.imshow(oc_img_mad_sc_inv_ctrpt, color_continuous_scale='gray')
                    fig.update_layout(
                        title="Center Points MAD After Rescaling"
                    )
                    fig.show()
                    ...
                    fig = px.imshow(oc_img_acc_mad_sc, color_continuous_scale='gray')
                    fig.update_layout(
                        title="ROI After Rescaling"
                    )
                    fig.show()
                    fig = px.imshow(oc_img_acc_mad_sc_sum_over_sizes, color_continuous_scale='gray')
                    fig.update_layout(
                        title="ROI After Rescaling: Sum Over Sizes"
                    )
                    fig.show()

                   # ROI mad's:

                    fig = px.imshow(oc_img_mad_sc_inv, color_continuous_scale='gray')
                    fig.update_layout(
                        title="ROI MAD After Rescaling"
                    )
                    fig.show()
